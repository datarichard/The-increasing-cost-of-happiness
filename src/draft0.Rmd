...
A fundamental question for governments and people is how much happiness does a dollar buy? The accepted view among economists and psychologists is that money and happiness increase together up to a point, after which there is little further gain from increasing wealth. While the location of this change point has been determined, and the cost of happiness reportedly ranges between USD\$60 to $95K, there has been no investigation as to whether the cost of happiness has increased or decreased over time. We tested the relationship between wealth and both happiness and life satisfaction using household economic data from Australia between 2002-2018. We discovered the cost of happiness has increased over those 18 years faster than inflation (i.e., cost of living). This result shows that inequalities in wealth may be driving inequities in happiness and wellbeing, with implications for health and recent government policy-goals to monitor and improve wellbeing. (150 words)  
...
Where $x_i$ was an individual's household wealth, and $X_i$ were covariates for age, age^2^, sex and education.  
...
Average life satisfaction levels were very steady between 2002-2018, while average happiness score decreased slightly over the 18 years. The proportions of each sex and relationship status were stable over time, as were the average household size and SEIFA index. However age and chronic health conditions tended to increase over time, as expected in an aging sample. Education levels (i.e., graduate percentage) also tended to increase over time, while changes in the workforce varied both up and down.  
...
##### Figure 4. Estimated happiness (SD units) at $50K and $80K from 2002-2018 
```{r figure_4}
predicted_fits <- list()
for (pathtofile in path_to_fits) {
...
predicted_fits %>%
  ...
  facet_wrap(~income) +
  labs(caption = "Source: Household disposable income, HILDA, Melbourne University", 
       x = "", y = "") -> p
...
```
Figure 4 legend: The difference (∆) in happiness between a household earning $50K per year and a household earning $80K per year has increased since 2009.
```  

<br>

Figure 5 presents house income levels adjusted for the Australian population (by age, sex, marital status, labour force participation and region). It shows the cost of happiness increased faster than household income between 2002 and 2018. As a result, a smaller percentage of the Australian population in 2018 achieved a level of financial security on which their happiness was no longer dependent on their income, relative to 2002.  

<br>

##### Figure 5.  
```{r figure_5, fig.width=7, fig.height=6}
...
ggplot(df.plot, aes(x = year, y = value, color = key)) +
  ...
  labs(title = "Wealth & happiness in Australia between 2002-2018",
       caption = "Source: HILDA sample weighted for Australian population
                  Household equivalised income per person (modified OECD scale)
                  Excluding full-time students and topcoded individuals",
       x = "  ", y = "") +
...
```
Figure 5 legend: Household income has stagnated in Australia since the GFC while the cost of happiness has increased. Consequently fewer Australians have a household income exceeding the cost of happiness in 2018.
```  
